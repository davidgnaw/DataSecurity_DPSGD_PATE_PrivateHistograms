...
tf.app.flags.DEFINE_integer('batch_size', 10, """Nb of images in a batch.""") #for runtime
tf.app.flags.DEFINE_integer('epochs_per_decay', 10, """Nb epochs per decay""") #for runtime
...
def _input_placeholder():
  ...
  tf.compat.v1.disable_eager_execution() #added for disabling in v2 tensorflow
...
def train(images, labels, ckpt_path, dropout=False):
  ...
  with tf.Graph().as_default():
    ...
    for step in xrange(FLAGS.max_steps):
      ...
      # Sanity check
      #assertion causes lots of troubles in long runtime so will comment out for now
